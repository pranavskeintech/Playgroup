      ...
      child: Column(
        ...
        children: [
          Container(
              ...
              child: Text("${searchData.length} Matches found",
                  style: TextStyle(
                      fontSize: 18,
                ...
                return Container(
                  child: ListTile(
                    leading: CircleAvatar(
                                          backgroundColor: Colors.white,
                      backgroundImage: searchData[index].profile != "null"
                          ? NetworkImage(Strings.imageUrl +
                              (searchData[index].profile ?? ""))
                          : AssetImage("assets/imgs/profile-user.png")
                              as ImageProvider,
                    ),
                    title: Text(searchData[index].childName ?? "",
    ...
    api.SearchChild(Strings.searchText, Strings.SelectedChild).then((response) {
      ...
      if (response.status == true) {
        ...
        AppUtils.createSnackBar(
            context, response.message ?? "Unable to Search Details");
        AppUtils.dismissprogress();
        // _btnController.stop();
      }
    }).catchError((onError) {
